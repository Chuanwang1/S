...
void Motor_Duoji_Init(void)
{
	...
	//设置IO引脚
	GPIO_InitStructure.GPIO_Pin = MOTOR_DUOJI_PIN;				 					//引脚配置
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP; 		 						//推挽输出
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;								//IO口速度为50MHz
	GPIO_Init(MOTOR_DUOJI_PORT, &GPIO_InitStructure);					 			//根据设定参数初始化
	GPIO_SetBits(MOTOR_DUOJI_PORT,MOTOR_DUOJI_PIN);						 			//初始输出高
	
	GPIO_InitStructure.GPIO_Pin = MOTOR_DUOJI_PIN_2;				 				//引脚配置
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP; 		 						//推挽输出
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;								//IO口速度为50MHz
	GPIO_Init(MOTOR_DUOJI_PORT, &GPIO_InitStructure);					 			//根据设定参数初始化
	GPIO_SetBits(MOTOR_DUOJI_PORT,MOTOR_DUOJI_PIN_2);						 		//初始输出高
	...
	TIM_TimeBaseInit(MOTOR_DUOJI_TIM, &TIM_TimeBaseStructure); 			//根据TIM_TimeBaseInitStruct中指定的参数初始化TIMx的时间基数单位
	...
	TIM_OC1Init(MOTOR_DUOJI_TIM, &TIM_OCInitStructure);  						//根据TIM_OCInitStruct中指定的参数初始化外设TIMx
	
	TIM_OCInitStructure.TIM_OCMode = TIM_OCMode_PWM2; 							//选择定时器模式:TIM脉冲宽度调制模式2
	TIM_OCInitStructure.TIM_OutputState = TIM_OutputState_Enable;		//比较输出使能
	TIM_OCInitStructure.TIM_Pulse = 0; 															//设置待装入捕获比较寄存器的脉冲值
	TIM_OCInitStructure.TIM_OCPolarity = TIM_OCPolarity_High; 			//输出极性:TIM输出比较极性高
	TIM_OC4Init(MOTOR_DUOJI_TIM, &TIM_OCInitStructure);  						//根据TIM_OCInitStruct中指定的参数初始化外设TIMx

	TIM_CtrlPWMOutputs(MOTOR_DUOJI_TIM,ENABLE);											//MOE 主输出使能
